...
//EXTRA IMPORT PRA FUNCIONAR OS PERSONAGEMS SAINDO DO JOGO LOL
...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		@:privateAccess
        var bfFrame = boyfriend._frame;
        
        if (bfFrame == null || bfFrame.frame == null) return;
            
        var rect = new Rectangle(bfFrame.frame.x, bfFrame.frame.y, bfFrame.frame.width, bfFrame.frame.height);
        
        bfScrollWin.scrollRect = rect;
        bfScrollWin.x = (((bfFrame.offset.x) - (boyfriend.offset.x / 2)) * bfScrollWin.scaleX);
        bfScrollWin.y = (((bfFrame.offset.y) - (boyfriend.offset.y / 2)) * bfScrollWin.scaleY);
	}

	function popupWindowDadShit(?xOffset:Int, ?yOffset:Int) {
		windowDad = Lib.application.createWindow({
			title: 'nome de exemplo lol', //nome da sua janela se voce quiser
			width: 1280, //tamanho da sua janela
			height: 720, //tamanho da sua janela
			borderless: true, //false = sem bordas do windows | true = com bordas do windows
			alwaysOnTop: true, //sempre aparece na frente do jogo e bem mais legal deixar isso como true pra ter melhor experiencia lol
		...
		dadWin.graphics.beginBitmapFill(dad.pixels, new Matrix());
		...
		dadScrollWin.scaleX = 1.4; //tamanho
		dadScrollWin.scaleY = 1.4; //tamanho
		dadGroup.visible = false; //isso faz pra o opponente ficar invisivel
		Lib.application.window.onClose.add(function() {
			windowDad.close();
		});
		FlxTransWindow.getWindowsTransparent(); //pra janela ficar transparente
		Application.current.window.focus();
	}

	function popupWindowBFShit(?xOffset:Int, ?yOffset:Int) { //esse e o meu exemplo
		windowBF = Lib.application.createWindow({
			title: 'esse e VOCE!!11 SIM VOCE MESMO !1!!11',
			width: 1280,
			height: 720,
			borderless: true,
			alwaysOnTop: true,
		...
		windowBF.x += xOffset;
		windowBF.y += yOffset;
		windowBF.stage.color = 0xFF010101;

		bfWin.graphics.beginBitmapFill(boyfriend.pixels, new Matrix());
		bfWin.graphics.drawRect(0, 0, boyfriend.pixels.width, boyfriend.pixels.height);
		bfWin.graphics.endFill();
		bfScrollWin.scrollRect = new Rectangle();
		windowBF.stage.addChild(bfScrollWin);
		bfScrollWin.addChild(bfWin);
		bfScrollWin.scaleX = 1.4;
		bfScrollWin.scaleY = 1.4;
